<template>
  <v-container>
    <v-row>
      <v-col>
        <v-textarea label="Comentario" v-model="comentario"></v-textarea>
      </v-col>
    </v-row>
    <v-row>
      <v-col>
    ...
    save() {
      this.$apollo
        .mutate({
          ...
          mutation: gql`
            mutation save(
              $tipoSelected: ENUM_SEGUIMIENTO_TIPO!
              $comentario: String!
            ) {
              createSeguimiento(
                input: {
                  data: { tipo: $tipoSelected, comentario: $comentario }
                }
              ) {
                seguimiento {
                  id
